...
//rescales duz to be within [0, xe_max] in place. 
int thermodynamics_control_rescaling(
				struct thermodynamics * pth,
				double xez,
				double * duz){
	...
	xe_max = 1.+(pth->YHe/2.0)*(1-pth->YHe);

	thermodynamics_root_bracket(&thermodynamics_rescaling_function, xez, xe_max, 
			&xbound1, &xbound2, pth->error_message);

	horizontal_shift = thermodynamics_root_bisect(&thermodynamics_rescaling_function, xez, xe_max,
			xbound1, xbound2, pth->error_message);

	expit = 1.0/(1.0 + exp(-((*duz)+horizontal_shift)));
	*duz = expit*xe_max - xez;
	return 0;
}
...
#define NTRY 50 //number of steps to take while bracketing
#define FACTOR 1.6
...
int thermodynamics_workspace_init(
  ...
  ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_reio] = ppr->recfast_delta_z_reio;
	...
	/* In cases where we need to interpolate for xe perturbations, we fill the derivative tables now*/
	if(pth->xe_pert_type==xe_pert_basis){
	...
	if(!(pth->is_shooting)){
		switch(pth->xe_pert_type){
			...
			case xe_pert_basis:
				...
				ptdw->xe_pert = duz;
				ptdw->x_reio = ptdw->x_fid*(1.0 + ptdw->xe_pert);
				break;
